...
`WordOverlap` is a trivial in-process memory system that ships with the package, and `demo` is
...
evaluation: demo at memrank-demo@v1+def0, 5 task(s), cleared per-group
  ...
  latency.ingest.p50     0.005  decided by memrank [whole run]
    milliseconds, memrank's own clock, over 1 sample(s)
  latency.ingest.p95     0.005  decided by memrank [whole run]
    milliseconds, memrank's own clock, over 1 sample(s)
  latency.retrieve.p50   0.007  decided by memrank [whole run]
    milliseconds, memrank's own clock, over 5 sample(s)
  latency.retrieve.p95   0.016  decided by memrank [whole run]
    milliseconds, memrank's own clock, over 5 sample(s)
...
for, there is no class to write: use its class instead -- `AtomicMemory`, `Hindsight`, `Mem0`
or `Supermemory` from `memrank.systems`, with `base_url=` (and `api_key=` for the two that
authenticate) where `WordOverlap()` goes. Each also reads its own environment variable when
the argument is left out -- `ATOMICMEMORY_API_URL`, `HINDSIGHT_API_URL`,
`SUPERMEMORY_BASE_URL`, `MEM0_HTTP_URL`.

`memrank.catalog()` prints everything that ships -- every system with what it needs, every
evaluation with what it measures -- so you do not have to know a name to find one. The same
things are reachable by string, `memrank.system("atomicmemory")` and
`memrank.evaluation("demo")`, which is the form a config file and the command line have;
`examples/05-against-a-baseline/` says when each form is the right one.
...
control = memrank.system("no-context")
