...
A [system](../reference/system.md) is the thing under test. Memrank ships nine of them, so you
can take a number before you have wired anything up, and so every engine row has controls beside
it. One page each.
...
| [FixedContext](fixed-context.md) | `fixed-context` | a control that hands over the corpus unranked, cut off at the same token budget the engine beside it was held to | nothing |
...
`memrank.catalog()` prints the same list at runtime, and `word-overlap` and the three controls
are the four that need nothing at all.
