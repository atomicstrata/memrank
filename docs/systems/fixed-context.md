...
A control that hands over the corpus unranked, cut off at the same token budget the system
...
reader, the run's shared token budget cuts it at the same size the system under test was
allowed, which is what makes the comparison a fair one.

## Why it matters

It separates *selecting well* from *sending more*. Held to the same number of tokens, a system
