...
> **Not uploaded yet.** The `memrank` name is held on PyPI, but the first functional release
> has not been published to it -- the only version on the index is a yanked placeholder that
> installs nothing, so the command above currently fails with `No matching distribution found`.
> Until the release lands, use the source install below. This page will not change when it
> does: the command above is the one that will work.
...
<details>
<summary>Installing from source (contributors, and until the release lands)</summary>
