...
- **Install:** `uv tool install memrank`. The name is held on PyPI and the first functional
  release is not uploaded yet; until it is, `uv tool install git+https://github.com/atomicstrata/memrank`
  installs the same tool from the public repository.
