...
So the image we build is not ours to hand out, and we do not publish it. The `Dockerfile` fetches
...
or `GROQ_API_KEY`, and then never calls it on memrank's path: ingest posts pre-chunked content to
